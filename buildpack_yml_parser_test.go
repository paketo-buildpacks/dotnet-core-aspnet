...
func testBuildpackYMLParser(t *testing.T, context spec.G, it spec.S) {
	var (
		...
		path   string
		parser dotnetcoreaspnet.BuildpackYMLParser
	)
...
`)
		...
		parser = dotnetcoreaspnet.NewBuildpackYMLParser()
	})
	...
	context("ParseVersion", func() {
		it("parses the dotnet framework version from a buildpack.yml file", func() {
